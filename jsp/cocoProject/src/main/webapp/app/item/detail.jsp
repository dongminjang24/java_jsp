                        ...
                        <div class="col detail_content_img">
                            <div id="carouselExampleCaptions" class="carousel slide" data-bs-ride="false">
                                <div class="carousel-indicators">
									<c:forEach var="file" items="${itemImages}" varStatus="status">
									    <button type="button" data-bs-target="#carouselExampleCaptions" data-bs-slide-to="${status.index}" class="${status.index == 0 ? 'active' : ''} change" id="change${status.index + 1}" ${status.index == 0 ? 'aria-current="true"' : ''} aria-label="Slide ${status.index + 1}"></button>
									</c:forEach>
                                </div>
									    ...
									    <c:when test="${not empty itemImages and fn:length(itemImages) > 0}">
									        <c:forEach var="file" items="${itemImages}" varStatus="status">
											    <div class="carousel-item ${status.index == 0 ? 'active' : ''}">
											        <img src="/item/findFileName.it?fileName=${file.getFileSystemName()}" class="d-block w-100" style="height:300px" alt="Image" />
											    </div>
											</c:forEach>
									    </c:when>
									    <c:otherwise>
